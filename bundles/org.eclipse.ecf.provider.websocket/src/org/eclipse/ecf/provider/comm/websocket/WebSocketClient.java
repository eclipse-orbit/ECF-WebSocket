...
public final class WebSocketClient implements ISynchAsynchConnection {
	public static final String PROTOCOL = "ecfws"; //$NON-NLS-1$
	 ...
	 */
	private void setSocket(WebSocket s) throws SocketException {
		socket = s;
	}
	
	public WebSocketClient(WebSocket aSocket, ObjectInputStream iStream, ObjectOutputStream oStream, ISynchAsynchEventHandler handler) throws Exception {
		this(aSocket, iStream, oStream, handler, DEFAULT_MAX_BUFFER_MSG);
	}

	public WebSocketClient(WebSocket aSocket, ObjectInputStream iStream, ObjectOutputStream oStream, ISynchAsynchEventHandler handler, int maxmsgs) throws Exception {
	...
	public WebSocketClient(ISynchAsynchEventHandler handler, int keepAlive) throws Exception {
		if (handler == null)
...
//		return retID;
		...
		ID retID = null;
		try {
			retID = IDFactory.getDefault().createStringID(PROTOCOL + "://" + "localhost" //$NON-NLS-1$
					+ ":" + "0"); //$NON-NLS-1$
		} catch (final Exception e) {
			traceStack("Exception in getLocalID()", e); //$NON-NLS-1$
			return null;
	...
	public synchronized Object connect(ID remote, Object data, int timeout) throws ECFException {
		...
		final WebSocket s = createConnectSocket(anURI, timeout);
	...
	void handleException(Throwable e) {
		...
		synchronized (WebSocketClient.this) {
			WebSocketClient.this.notifyAll();
		}
	}
				...
				while (!queue.isStopped()) {
					try {
						...
						synchronized (pingLock) {
							...
							if (waitForPing)
								throw new IOException("remote not reachable by ping"); //$NON-NLS-1$
						}
					} catch (final Exception e) {
						handleException(e);
						break;
	...
	protected void traceStack(String msg, Throwable e) {
		Trace.catching(ProviderPlugin.PLUGIN_ID, ECFProviderDebugOptions.EXCEPTIONS_CATCHING, WebSocketClient.class, msg, e);
	}
